# Registers the Crystal language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koCrystalLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Crystal")
    registry.registerLanguage(KoCrystalLanguage())

class KoCrystalLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Crystal"
    lexresLangName = "Crystal"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "ac2c87f6-50a8-42fd-a7a8-1496c83f9e93"
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Crystal'}
